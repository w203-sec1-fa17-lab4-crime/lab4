...
#I think adding a scatterplot matrix here would be appropriate to 1) check linear relationships between our variables 2) to identify strength of linear correlation

```{r}
scatterplotMatrix(~ crmrte + prbarr + prbconv + polpc + density + taxpc + pctmin80 + pctymle, data = data)
cor(data[, c("crmrte", "prbarr", "prbconv", "polpc", "density", "taxpc", "pctmin80", "pctymle")])
cor(data[!(names(data) %in% c("X", "county", "year"))], use = "complete.obs")
# density, taxpc, prbarr, prbconv, urban for model 1 
```
...
density, taxpc, prbarr, prbconv for model 1 

# Model 2: key explanatory variables and covariates that increase accuracy without introducing bias
model 1 plus variables 
1. Zero-conditional Mean Bias
2. Omitted Variable Bias

try to include variables that have decent correlation, but not highly correlated with other variables
