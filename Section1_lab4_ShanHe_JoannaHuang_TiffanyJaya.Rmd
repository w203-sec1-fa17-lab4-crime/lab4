...
```
Note that although we don't have any counties that are both in west and central, as expected, we see some counties that neither in west or central. In order to consider the effect on different regions, we need to create another column that has 1 when the county isn't in the west or central and 0 when it is west or central.  

```{r}
# create indicator variable other_region
data$other_region <- ifelse(data$west + data$central == 1, 0, 1)
```
...
```{r}
m3 <- lm(log(crmrte) ~ prbarr + prbconv + prbpris + avgsen + log(polpc) + density + taxpc + west + central + other_region + urban + pctmin80 + log(wcon)
...
violation of zero-conditional mean & homoskedasticity due to one data point. 
