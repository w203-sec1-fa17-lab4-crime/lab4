...
```{r}
# correlation matrix of explanatory variables against other variables of potential interest
ind.vars.names <- c("prbarr", "prbconv", "prbpris", "avgsen", "polpc", "log.polpc")
ind.vars <- subset(data, select=ind.vars.names)
other.vars <- subset(data, select=!names(data) %in% c(ind.vars.names, "X", "county", "year", "crmrte"))
cm <- as.data.frame(as.table(cor(ind.vars, other.vars)))
cm[order(-abs(cm$Freq)),]
```
